...
class Trainer(object):
    ...
    def validate(self):
        ...
        self.mean_loss = total_loss.item() / len(self.val_loader)
        if np.isnan(self.mean_loss):
            raise ValueError("self.mean_loss is nan while validation")
        if self.epoch==1: self.best_mean_loss = self.mean_loss
        torch.save(
            {
        ...
        if training:
            ...
            del input_dict, result, total_loss
    ...
    def train(self):
        ...
        for self.epoch in range(start_epoch, CO.max_epoch):
            ...
            self.train_epoch()
            # if self.epoch < CO.max_epoch//2: continue
            if self.epoch%2 !=0 and self.epoch!=(CO.max_epoch-1): continue
            self.validate()
